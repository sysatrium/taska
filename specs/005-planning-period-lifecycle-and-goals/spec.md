...
Head of Product открывает planning-модуль продукта и переходит в раздел подготовки планирования, где рядом доступны глобальные planning entities: Команды и «Периоды планирования». Из раздела «Периоды планирования» пользователь создаёт новый период, задаёт его название, даты и goals, сохраняет период как `draft`, затем подготавливает его к запуску и переводит по lifecycle-статусам.
...
Сценарий: Head of Product из planning-модуля через раздел подготовки планирования и раздел «Периоды планирования» создаёт новый период и переводит его в `open`
  Дано Head of Product находится в planning-модуле продукта
  И Head of Product перешёл в раздел подготовки планирования без использования скрытого URL
  И Head of Product открыл раздел «Периоды планирования»
  ...
  И открывает details screen созданного периода из списка «Периоды планирования»
  И вводит goals с форматированием
  И видит, что период проходит readiness hints для перехода в open
  И выполняет действие Open
  Тогда planning period сохраняется и отображается в списке «Периоды планирования»
...
- Допустимые статусы: `draft`, `open`, `closed`, `cancelled`.
- Допустимы только следующие переходы: `draft → open`, `draft → cancelled`, `open → closed`.
...
- В статусах `draft`, `open` Head of Product может свободно редактировать goals. В рамках данной фичи система не хранит историю изменений goals и не помечает отдельно сделанные изменения.
...
AC01 — Невалидный период с одинаковой датой начала и окончания
...
AC02 — Невалидный период, полностью лежащий в прошлом
  Дано текущая дата — T
...
AC03 — Невозможность создать пересекающийся период того же уровня
...
AC04 — Goals, состоящие только из пробелов, не позволяют открыть период
...
AC05 — Период в статусе cancelled нельзя вернуть в работу
...
AC06 — Период в статусе closed нельзя переоткрыть
...
AC07 — Пустой список периодов показывает empty state с CTA
...
AC08 - Head of Product может открыть список planning periods из основного UI
...
AC09 - Список planning periods показывает статусы периодов
...
AC10 - Head of Product может создать planning period в статусе draft
...
AC11 - Head of Product может открыть details screen planning period из списка
...
AC12 - Head of Product может редактировать metadata и goals в draft
...
AC13 - Head of Product не может перевести period в open без goals
  Дано существует planning period в статусе draft
...
AC14 - Head of Product видит readiness hints перед переходом в open
...
AC15 - Head of Product может перевести period из draft в open
  Дано существует planning period в статусе draft
...
AC23 - Для closed lifecycle actions недоступны
  Дано существует planning period в статусе closed
  Когда Head of Product открывает details screen planning period
  Тогда lifecycle actions недоступны

AC24 - Для cancelled lifecycle actions недоступны
  Дано существует planning period в статусе cancelled
  Когда Head of Product открывает details screen planning period
  Тогда lifecycle actions недоступны

AC25 - Details screen показывает status-aware actions
...
AC26 - Empty state не блокирует основной путь без обратной связи
...
AC27 - Loading state не блокирует основной путь без обратной связи
...
AC28 - Error state не блокирует основной путь без обратной связи
  Дано Head of Product использует экран списка planning periods или details screen planning period
